<template lang="pug">
  .v-dialog(:class='{active: show, [`v-dialog-${item.size}`]: item.size, [`v-dialog-${item.class}`]: item.class}')
...
.v-dialog:target, .v-dialog.active {
  display: flex;
  display: -ms-flexbox;
  opacity: 1;
  z-index: 400;
...
.v-dialog:target .v-dialog-overlay, .v-dialog.active .v-dialog-overlay {
...
.v-dialog:target .v-dialog-container, .v-dialog.active .v-dialog-container {
